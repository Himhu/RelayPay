layui.define(['message', 'table', 'jquery', 'element', 'yaml', 'form', 'tab', 'menu', 'frame', 'theme', 'convert'],
	function(exports) {
		...
		var body = $('body');
		var menuReady = false;
		var pendingMobileCollapse = false;
			...
			this.bodyRender = function(param) {
				...
				if (isMuiltTab(param) === "true" || isMuiltTab(param) === true) {
					...
					sideMenu.click(function(dom, data) {
						var isHomeClick = data.openType === '_home'
							|| data.menuId == param.tab.index.id
							|| data.menuUrl == param.tab.index.href;
						if (isHomeClick) {
							element.tabChange(bodyTab.option.elem, param.tab.index.id);
							sessionStorage.setItem(bodyTab.option.elem + "-pear-tab-data-current", param.tab.index.id);
							bodyTab.positionTab();
							compatible();
							return;
						}
					...
					sideMenu.click(function(dom, data) {
						var isHomeClick = data.openType === '_home'
							...
							|| data.menuUrl == param.tab.index.href;
						if (isHomeClick) {
							bodyFrame.changePage(param.tab.index.href, true);
							compatible();
							return;
						}
		...
		function collapseMenuForMobile() {
			if ($(window).width() > 768) {
				pendingMobileCollapse = false;
				syncMobileLayoutState();
				return;
			}
			if (!menuReady) {
				pendingMobileCollapse = true;
				return;
			}
			pendingMobileCollapse = false;
			var admin = $(".pear-admin");
		...
		function syncMobileLayoutState() {
			var admin = $(".pear-admin");
			if ($(window).width() <= 768 && !admin.is(".pear-mini")) {
				$('body').addClass('pear-mobile-lock');
			} else {
				$('body').removeClass('pear-mobile-lock');
			}
		}
