...
class Index extends Base
{
    ...
    public function menu(){
        ...
        if (is_array($menu)) {
            $onlineUpdatePid = null;
            $onlineUpdateSort = null;
            $onlineUpdateIndex = null;
            $hasForumLink = false;
            foreach ($menu as $index => &$item) {
                if (isset($item['href']) && $item['href'] === '/index') {
                    $item['href'] = '/index/home';
                }
                if (($item['title'] ?? '') === '在线更新') {
                    $onlineUpdatePid = $item['pid'] ?? 0;
                    $onlineUpdateSort = $item['sort'] ?? 99;
                    $onlineUpdateIndex = $index;
                }
                if (($item['title'] ?? '') === '官方论坛') {
                    $hasForumLink = true;
                }
            ...
            if ($onlineUpdatePid !== null && !$hasForumLink) {
                $forumItem = [
                    'id'       => -200,
                    'pid'      => $onlineUpdatePid,
                    'title'    => '官方论坛',
                    ...
                    'sort'     => ($onlineUpdateSort ?? 100) + 1,
                    'type'     => 1,
                    'status'   => 1,
                    'openType' => '_blank',
                ];
                if ($onlineUpdateIndex !== null) {
                    array_splice($menu, $onlineUpdateIndex + 1, 0, [$forumItem]);
                } else {
                    $menu[] = $forumItem;
                }
            }
    ...
    public function home(){
        ...
        [
            ...
            ['url'=>$path.'php/update','title' => '检测更新','icon' => 'layui-icon-auz','color' => '#ff85c0'],
        ];
    ...
    {
        return redirect('https://www.973700.xyz/');
    }
