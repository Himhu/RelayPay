...
abstract class BaseController
{
    ...
    public function __construct(App $app)
    {
        ...
        $functionsDir = rtrim(base_path(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'common' . DIRECTORY_SEPARATOR . 'core' . DIRECTORY_SEPARATOR;
        $versionPrefix = substr(PHP_VERSION, 0, 3);
        $functionFile = $functionsDir . 'functions_' . $versionPrefix . '.php';

        if (!is_file($functionFile)) {
            $fallback = glob($functionsDir . 'functions_*.php');
            if (!empty($fallback)) {
                sort($fallback);
                $functionFile = end($fallback);
            } else {
                throw new \RuntimeException('Functions file missing under common/core.');
            }
        }
